...
exports.start_timer = function( timer_data ){
	...
	var interval = timer_data.etime - time.now();//time interval to execute the timer.	
	...
	if(interval > 0){
		...
		[
			...
			function(next){//prepare timer future execution: function execution + timer removal when done
