 ...
 */
exports.route = function( request, response, http_post_str){
     
    pathname = request._parsedUrl.pathname.replace("/",""); 
    if( endpoint[pathname] ){       	
		
		console.log("router.route()");
		//endpoint[pathname](request, response);
    }else{
		//console.log("No request handler found for "+ pathname);
		http_resp.writeHead(CONST.HTTP_NOT_FOUND(),{"Content-type":"text/html"});
