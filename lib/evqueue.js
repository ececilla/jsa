...
function default_ev_handler(msg, rcpts){
	...
	if( !(rcpts = rcpts || msg.ev_ctx.doc.rcpts) )//rcpts parameter takes precedence over document rcpts
		return;
...
api.on("ev_api_create", function( msg, rcpts ){
	...
	if( !(rcpts = rcpts || msg.ev_ctx.doc.rcpts) )
		return;
...
function save_and_push( msg, ret_handler ){
	...
	msg._id = Math.random().toString(36).substring(2,10);
