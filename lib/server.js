 ...
 */
exports.api.init.execute = function(ret_handler){
	/*
	...
	*/
	srv.config.db.default_catalogs.push("foo");
	ret_handler();	
}
			...
			httpsrv.listen( srv.config.app.httpport, srv.config.app.httpaddr, function(){														
				...
				[										
					function(next){		
						...
						srv.api.init.execute(function(){
							
							console.log("**Executed api.init.execute");										
							console.log("**Remote operations: {%s}", Object.keys(jsa.api.docs).join());																																										
							next();
						});
					},
					function(next){
