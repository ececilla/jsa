...
for( key in api.remote ){
	...
	(function(){
		...
		var scopekey = key;		
		srv.config.runtime.ops[scopekey] = 1;
		srv.api.docs[scopekey] = function(params, ret_handler){
 ...
 * ev_eq_push
 */
exports.eq.events.on = function( ev_type, ret_handler ){
 ...
 */
exports.eq.events.emit = function( ev_type, params, rcpts ){
 ...
 * 
 */
exports.events.on = function( ev_type, ret_handler ){
 ...
 * 
 */
var emit = exports.events.emit =  function( ev_type, params, rcpts ){
...
exports.api.newop = function( fname, f_handler ){ 
	...
	srv.api.docs[fname] = function( params, ret_handler ){
		
		api.remote[fname](params, ret_handler);
	}
	
	eq.api.listen( ev_name );
	srv.config.runtime.ops[fname] = 1;
}
			...
			httpsrv.listen( srv.config.app.httpport, srv.config.app.httpaddr, function(){														
				...
				[										
					function(next){		
						...
						async.series( srv.api.init.execute, function(err, results){		
													
							console.log("**Executed jsa.api.init.execute");										
							console.log("**Remote operations: {%s}", Object.keys(srv.api.docs).join());																																										
							next();
						});
					},
					function(next){
