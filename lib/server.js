 ...
 */
exports.api.docs.newop = function( fname, f_handler ){ //function(params)
	
	api.remote[fname] = function( params, ret_handler){
		
		f_handler(params, ret_handler);
		
		if(params.cancel_default_event ==  undefined)
			api.emit( "ev_" + fname, params );
	}
	
	eq.api.listen( "ev_" + fname );
}
