...
srv.api.config.add_plugout = function( proc_name, plugout_name, func_handler){
	
	sandbox.add_plugout(proc_name, plugout_name, func_handler );
}
		...
		for( i in srv.config.db.system_catalogs ){
			for(op in api.remote){
				(function(){
					...
					var localop = op;
					var localcatalog = srv.config.db.system_catalogs[i];					
					sandbox.add_constraint_pre(localop, "not_catalog_" + localop + localcatalog, sandbox.constraints.not_catalog, localcatalog);
				})();
			}
		}
		...
		sandbox.add_constraint_pre("register","param_user",sandbox.constraints.is_required("user"))
			   ...
			   .add_constraint_pre("create","param_uid",sandbox.constraints.is_required("uid"))
	  		   ...
	  		   .add_constraint_pre("dispose","param_uid",sandbox.constraints.is_required("uid"))	   		   
	   		   ...
	   		   .add_plugin("create","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("dispose","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("dispose","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("join","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("join","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("unjoin","url_transform", sandbox.plugins.url_transform)	  
	   		   .add_plugin("unjoin","external_config", sandbox.plugins.external_config) 		   
	   		   .add_plugin("remove","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("remove","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("set","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("set","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("push","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("push","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("pop","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("pop","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("shift","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("shift","external_config", sandbox.plugins.external_config)
	   		   .add_plugin("get","url_transform", sandbox.plugins.url_transform)
	   		   .add_plugin("get","external_config", sandbox.plugins.external_config);
...
/*
 * Register listeners for eventmanager events. Eventqueue event list: ev_eq_push
 */
srv.evmngr.events.on = function( ev_type, ret_handler ){
				
	evmngr.on(ev_type, ret_handler);
	return srv.evmngr.events;	
}
