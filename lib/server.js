...
/*
 * http server variable enhanced via connect middleware.
 */
var httpsrv =  exports.connect = connect();//.createServer();
httpsrv.use( connect.json() );
...
exports.start = function( econfig, ret_handler ){		
	...
	async.series([
		...
		function(next){//Connect to message bus
		...
		function(next){//Process request through connect middleware

			 httpsrv.use( function (request, response, next){
				    ...
				    if( request.headers["content-type"] == "application/json" ){
						...
						request.on("data", function( chunk ){
						    ...
						    if(data.length > 1e6){
						    	...
						    	response.writeHead(CONST.HTTP_ENTITY_TOO_LARGE(),{"Content-type":"text/html"});
						    	response.end("413 Entity too large");
						    }
