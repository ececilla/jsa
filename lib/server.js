...
srv.config = { 	system:{}, 
				events:{}, 				
				db:{
					default_catalogs : ["docs", "events", "timers"]
				} 
			};
srv.init = {};			
srv.api = { config:{primitives:{}}, events:{} };
srv.eq = { events:{} };
...
for( key in api.remote ){
	...
	(function(){
		...
		var scopekey = key;		
		srv.api.config.primitives[scopekey] = CONST.ENABLE();
		srv.api[scopekey] = function(params, ret_handler){
							
			api.remote[scopekey](params, ret_handler?ret_handler:function(err,val){
...
srv.api.config.enable_primitives = function(){
	
	for(key in srv.api.config.primitives)
		srv.api.config.primitives[key] = CONST.ENABLE();
}

srv.api.config.disable_primitives = function(){
	
	for(key in srv.api.config.primitives)
		srv.api.config.primitives[key] = CONST.DISABLE();	
}
...
srv.init.execute = [
	...
	function( end_handler ){
		/*
		srv.config.events.catalog = {};
		srv.config.events.catalog["docs"].mode = 1; //all event_types have the same event_mode for docs belonging to docs catalog.
		
		
		srv.config.events.catalog["docs"].operation = {};
		//catalog op
		srv.config.events.catalog["docs"].operation["create"].mode = 1;
		//doc ops
		srv.config.events.catalog["docs"].operation["join"].mode = 1;
		srv.config.events.catalog["docs"].operation["unjoin"].mode = 1;
		srv.config.events.catalog["docs"].operation["add"].mode = 0;
		srv.config.events.catalog["docs"].operation["remove"].mode = 0;	
		
		//field ops
		srv.config.events.catalog["docs"].operation["set"].mode = 0;
		srv.config.events.catalog["docs"].operation["set"].field = {};
		srv.config.events.catalog["docs"].operation["set"].field["x"].mode = 1;
		srv.config.events.catalog["docs"].operation["set"].field["y"].mode = 1;
		srv.config.events.catalog["docs"].operation["set"].field["z"].mode = 1;
		
		srv.config.events.catalog["docs"].operation["add"].field["a"].mode = 1;
		srv.config.events.catalog["docs"].operation["add"].field["a.b"].mode = 1;
		srv.config.events.catalog["docs"].operation["add"].field["a.b.c"].mode = 1;
		
		srv.config.events.catalog["docs"].operation["push"].mode = 1;
		srv.config.events.catalog["docs"].operation["pop"].mode = 1;
		srv.config.events.catalog["docs"].operation["shift"].mode = 1;
		srv.config.events.catalog["docs"].operation["unshift"].mode = 1;
		
		
		srv.config.events["create"] = {docs:0};	
		*/			
...
srv.api.events.cancel_default_event = function(){
 ...
 */
srv.api.events.emit = function( ev_type, params, rcpts ){
...
/*
 * Register listeners for eventqueue events. Eventqueue event list:
 * 
 * ev_eq_push
 */
 ...
 */
srv.eq.events.emit = function( ev_type, params, rcpts ){
			
	eq.emit( ev_type, params, rcpts );
}
 ...
 * 
 */
var emit = srv.events.emit =  function( ev_type, params, rcpts ){
...
srv.api.config.newop = function( fname, f_handler ){ 
	...
	var ev_name = "ev_api_" + fname;	
						
	api.remote[fname] = function( params, ret_handler){
		
		srv.api.events[ev_name] = {params:params, rcpts:undefined};
	...
	srv.api[fname] = function( params, ret_handler ){
		
		api.remote[fname](params, ret_handler);
	}
	
	srv.api.config.primitives[fname] = CONST.ENABLE();
			...
			httpsrv.listen( srv.config.app.httpport, srv.config.app.httpaddr, function(){														
				...
				[										
					...
					function(next){
						...
						db.prepare(srv.config.db,function(){
							
							console.log("**Prepared db catalogs: {%s}", srv.config.db.default_catalogs.join() );
							next();
						});
					},
					function(next){	
