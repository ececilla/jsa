...
srv.api.config.add_plugin_mid = function( proc_name, plugin_name, func_handler, catalog_name ){
	
	sandbox.add_plugin_mid(proc_name, plugin_name, func_handler, catalog_name );
}
...
srv.api.config.newop = function( fname, f_handler ){ 						
	...
	var ret =  {
			...
			add_plugin_mid:function(plugin_name, func_handler, catalog_name){
				
				sandbox.add_plugin_mid( fname, plugin_name, func_handler, catalog_name );
				return ret;
			},
...
var init_arr = [
	...
	function( end_handler ){
		...
		sandbox.add_constraint_pre("create","disable_create",sandbox.constraints.is_disabled,"users")
	   	...
	   	//sandbox plugin_in, plugin_mid and plugin_out over default catalog
	   	sandbox.add_plugin_mid("create","notifying_doc",sandbox.plugins.notifying_doc)
...
var settings = exports.settings = function( econfig ){
	...
	try{																										
		console.log("settings resolve:"+require.resolve("../settings.json"));
	...
	if( request.method == "POST" ){	
	    ...
	    if( request.headers["content-type"] == "application/json" ){
			...
			request.on("end", function(){
			    ...
			    if(srv.config.app.auth_secret){ //Require authorization to perform this request
			    				    	
			    	var auth_hline = request.headers["authorization"];			    				    	
			    	var key = auth_hline && auth_hline.substring(auth_hline.indexOf("=",auth_hline)+1,auth_hline.length);			    				    				   
			    	var expected_key = util.hash( data + srv.config.app.auth_secret );
	    ...
	    response.write("<h1>Worizon api server</h1>");
	    response.write("<h4>Version: jsa@" + srv.config.app.version +"</h4>");
