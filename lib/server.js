...
exports.start = function( econfig, ret_handler ){		
	...
	async.series([
		...
		function(climb_boot){ //Set up connect middleware functions.
			...
			app.use(connect.logger("[:remote-addr - :uid] [:response-time ms]->:method :url :status :user-agent "));
			...
			app.use("/rpc",endpoint());//TODO: change endpoint by rpc.
			app.use(function(req,res,next){//if not /rpc path send not found error.
			...
			app.use(function(err,req,res,next){
				...
				if(err){
					...
					if( err.status == CONST.HTTP_BAD_REQUEST() ){
						
						res.end("400 Bad request.");
					}else if( err.status == CONST.HTTP_FORBIDDEN() ){
						
						res.end("403 Forbidden.");
					}else if( err.status == CONST.HTTP_NOT_FOUND() ){
