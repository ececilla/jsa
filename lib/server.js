...
/*
 * http server variable enhanced via connect filters.
 */
...
/*
 * Server setup with init functions. When all init stuff is done 
 * must call ret_handler to signal the end of the init function.
 * 
 */
var init_arr = [
...
exports.start = function( econfig, ret_handler ){		
	...
	async.series([
		...
		function(climb_boot){//Connect db
		...
		function(climb_boot){//Connect to message bus
		...
		function(climb_boot){//Check db collections
		...
		function(climb_boot){//Execute init functions.		
		...
		function(climb_boot){ //Set up connect middleware functions.
			
			express.logger.token("uid",function(req, res){
			...
			app.use( express.logger("[:remote-addr - :uid] [:response-time ms]->:method :url :status :user-agent ") );
			app.use( express.query() );
			app.use( express.urlencoded() );
			app.get( "/", express.basicAuth("worizon","gritxt"));			
			app.get( "/", function (req, res, next ){
				
				srv.config.app.uptime = util.uptime_hm();
				res.render("index", {config:srv.config.app});														
		    	/*
		    	res.writeHead( CONST.HTTP_OK(),{"Content-Type":"text/html"});
		    	res.write("<h1>JSA Api Server</h1>");
		    	res.write("<h4>Version: " + srv.config.app.name + "@" + srv.config.app.version + " [jsa@" + srv.config.app.jsa_version +"]</h4>");
		    	res.write("<h4>Uptime: " + util.uptime_hm() + "</h4>");
		    	res.end();								    				
		    	*/
			});
			app.post("/rpc",function(req,res,next){//check post content type and emit request event.
				if( req.headers["content-type"] == "application/x-www-form-urlencoded" ){
												
					emit("ev_srv_req", {pathname:req._parsedUrl.path, headers:req.headers} );												
					next();
			    }else{
					next({status:CONST.HTTP_BAD_REQUEST()});		
					
			    }
			})
			app.post("/rpc",reqsign(srv.config.app));						
			app.post("/rpc",endpoint());//TODO: change endpoint by rpc.			
			app.use(function(req,res,next){//if not "post /rpc" or "get /"  , send not found error.
				
				next({status:CONST.HTTP_NOT_FOUND()});
			});
			app.use(function(err,req,res,next){//create error handler js file.
		...
		function(next_start_step){//Load timers.	
			
			timers.begin(function(){
				
				console.log("**Reloading timers...");
				next_start_step();
			});
			
		},
						
		function(next_start_step){//Start httpd
						
			app.listen( srv.config.app.httpport, srv.config.app.httpaddr, function(){
				
				console.log("**Httpd listening on %s:%d", srv.config.app.httpaddr, srv.config.app.httpport);
				next_start_step();
			});
		},
		function(next_start_step){//start command line server
