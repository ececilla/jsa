...
exports.connect = function( params, ret_handler ){
	...
	exports.driver = db = new mongo.Db(params.dbname, new mongo.Server(params.dbhost,params.dbport,{auto_reconnect:true, socketOptions:{timeout:5000}}),{strict:true, safe:true, native_parser:true}); //,poolSize:5,socketOptions:{timeout:1}
...
var update = exports.update = function(col_name, id_str, upmodifier, ret_handler){
	...
	db.collection( col_name, function(err, collection){
	    ...
	    var ObjectID = require('mongodb').ObjectID;	
	    id_str = id_str instanceof ObjectID ? id_str:new ObjectID(id_str); 
			...
			async.series([
				function(next){
					...
					async.forEach(rcpts, function(rcpt,next){
						...
						select("users",rcpt.uid,function(err,user){//remove wid from all users wids list
							
							user.wids = _.reject(user.wids,function(wid){
								return wid === id_str;
							});							
							
							save("users",user,function(err){
								next(err);
							});
