...
var logger = log4js.getLogger("evmngr");
...
function default_ev_handler(msg, rcpts){	
	...
	if(msg.ev_ctx.config && msg.ev_ctx.config.ev_private)
		rcpts = rcpts && [rcpts[0]];	
		
	if(msg.ev_ctx.user)
		rcpts = _.reject( rcpts,function(rcpt){ return "" + rcpt.uid == "" + msg.ev_ctx.user._id});
	...
	async.series(
		[
			function(next){//save event into journal
				...
				if(srv.config.app && srv.config.app.ev_journal){			
