...
exports.constraints = {
	...
	is_joinable: function(ctx){
		...
		if(!ctx.doc.rcpts)
			return {code:-2, message:"No access permission: not joinable"};
	},
	...
	field_exists: function(ctx){
		...
		if(util.get_field(ctx.doc,ctx.params.fname)){
			...
			return {code:-3, message:"Field '" + ctx.params.fname+  "' already exists @" + ctx.params.catalog+":" + ctx.params.wid};
		}
	},
	...
	field_not_exists: function(ctx){
		...
		if(!util.get_field(ctx.doc,ctx.params.fname)){
			...
			return {code:-3, message:"Field '" + ctx.params.fname+  "' not exists @" + ctx.params.catalog+":" + ctx.params.wid};
		}
	}
