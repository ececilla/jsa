...
exports.execute = function( proc_name, params, ret_handler){
	...
	ctx.params.catalog = ctx.params.catalog || "docs";//srv.config.db.default_catalog;
...
exports.constraints = {
	...
	has_joined: function(ctx){
		...
		if( !ctx.doc.rcpts || ctx.doc.rcpts.indexOf(ctx.params.uid) == -1 ){
			...
			return {code:-2, message:"No access permission: not joined"};
		}
	},
	...
	not_joined: function(ctx){
		...
		if( ctx.doc.rcpts.indexOf(ctx.params.uid) != -1 ){
			...
			return {code:-2, message:"No access permission: already joined"};
		}
	},
	...
	writable_catalog : function(ctx){
		
		if( srv.config.db.writable_catalogs.indexOf(ctx.params.catalog) == -1 ){
			
			return {code:-2, message:"No access permission: not writable catalog"};
		}
	},
			...
			return function(ctx){
				...
				if(type == "array"){
					...
					if(typeof obj != "object" || !obj.push || !obj.pop || !obj.shift || !obj.unshift  ){
						...
						return {code:-4, message:"Wrong type: #" + ctx.params.catalog + "/" + ctx.params.wid + ":" + ctx.params.fname + " not array"};
					}
				}else{
