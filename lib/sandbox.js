...
function load_objects(ctx, ret_handler){					
	...
	async.parallel(
		[function(next){//load document
			...
			if(ctx.params && ctx.params.wid){
				
				db.select(ctx.params.catalog, ctx.params.wid, function(err,doc){
		...
		function(next){//load user
			...
			if(ctx.params && ctx.params.uid){
				...
				db.select("users",ctx.params.uid,function(err,user){
 ...
 * Add a plugin 
 */
exports.add_plugin_in = function(proc_name, plugin_name, func_handler){
 ...
 * Add a plugout
 */
exports.add_plugin_out = function(proc_name, plugout_name, func_handler){
	
	if(!plugouts[proc_name])
		plugouts[proc_name] = {};
	
	if(func_handler == undefined && typeof plugout_name == "function"){
		
		func_handler = plugout_name;
		plugout_name = util.generate_rstring(10);
	}	
	
	plugouts[proc_name][plugout_name] = function(ctx, end_handler){
...
exports.execute = function( proc_name, params, ret_handler){
	...
	/*			
	ctx.emit = function(rcpts){
			
		api.emit("ev_api_" + proc_name, ctx, rcpts);
	}
	*/
	//Execute plugins to transform ctx some way.
	async.forEachSeries(Object.keys(plugins[proc_name]||{}),function(plugin_name,next){
		
		plugins[proc_name][plugin_name](ctx, function(){
			
			if(ctx.err){
				
				ret_handler(ctx.err,null);
				return;
			}
			next();			
		});				
		...
		if(constraints[ctx.params.catalog]){
		    for( key in constraints[ctx.params.catalog].pre[proc_name] ){
				...
				var err = constraints[ctx.params.catalog].pre[proc_name][key](ctx);
				if(err){
		...
		load_objects(ctx, function(err){//document and user load
			...
			if(err){
				...
				if(constraints[ctx.params.catalog]){
					for( key in constraints[ctx.params.catalog].post[proc_name] ){
						...
						var err = constraints[ctx.params.catalog].post[proc_name][key](ctx);
						if(err){
				...
				api.remote[proc_name](ctx, function(err,val){
					...
					ctx.retval = util.clone(val);//return value returned over ctx.
					async.series([
					function(next){//execute plugouts: may modify ctx.doc, ctx.retval and ctx.payload
						
						async.forEachSeries(Object.keys(plugouts[proc_name]||{}),function(plugout_name,end_handler){
							
							plugouts[proc_name][plugout_name](ctx, end_handler);
...
exports.constraints = {
	...
	is_protected:function(protected_fname){
		...
		return function(ctx){
			...
			for(i=0; i < fname.length; i++){
				...
				if(fname[i] == protected_fname){
					
					return {code:-13, message:"Protected field not allowed as field name: " + fname[i] };
				}
			}
		}
	}			
