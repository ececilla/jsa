...
exports.execute = function( proc_name, params, ret_handler){
	...
	function(next_phase){ 	
		...
		if(ctx.config && ctx.config.emit){						
			...
			api.emit("ev_api_" + proc_name, ctx, undefined, ctx.config.tag);
		}
		next_phase();
	}
...
exports.constraints = {
	...
	are_required: function(fields){ 
		
			return function(ctx){				
				
				if(!_.every(fields,function(field){
					return _.contains(_.keys(ctx.params),field);
				})){
					return {code:-12, message: "All "  + nutil.inspect(fields)  + " parameters required"};	
				}											
		}
	},	
