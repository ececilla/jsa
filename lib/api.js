...
exports.remote = {
	...
	create : function( params, ret_handler ){
		...
		if( params.catalog && params.catalog == "events"){//hacer lo mismo para el timers catalog
		...
		async.series([
			function(next){
				...
				if(params.doc.rcpts && exports.rcpts){
					...
					exports.rcpts( params.doc, function(rcpts){
				...
				db.save(params.catalog, params.doc, function(err, doc){
					...
					if(!err){			
						...
						if(doc.etime){
								...
								time.create_remove_timer( {tid:id, etime:doc.etime, data:{catalog:params.catalog}} );												
						}															
					}else
						if(ret_handler)
	 ...
	 */
	dispose : function(params, ret_handler){
		...
		db.select(params.catalog, params.wid, function(err,doc){
			...
			if(!err && doc){
				...
				if(doc.uid == params.uid){
					
					db.removeById( params.catalog, params.wid, function(err, val){
				
						emit( "ev_api_dispose", params );
					...
					ret_handler({code:-7, message:"Not allowed, not document owner: @" + params.catalog + ":" + params.wid }, null);
				}
	 ...
	 */
	join : function( params, ret_handler ){
	 ...
	 */
	unjoin : function( params, ret_handler ){
	 ...
	 */
	add : function(params, ret_handler){
	 ...
	 */
	remove : function(params, ret_handler){
		
		if( !(params.wid && params.uid && params.fname ) ){
					
			ret_handler({code:-2, message:"Missing parameters:{wid:,uid:,fname:,(optional)catalog:,(optional)index:}"},null);
			return;		
		}
		...
		db.select(params.catalog, params.wid, function(err, doc){
			...
			if(!err && doc){
				...
				if( util.get_field(doc, params.fname) ){				
					...
					try{										
						util.del_field(doc, params.fname, params.index );
					}catch(err){
	 ...
	 */
	set : function( params, ret_handler){
		
		if( !(params.wid && params.uid && params.fname && params.value != undefined ) ){
					
			ret_handler({code:-2, message:"Missing parameters:{wid:,uid:,fname:,value:,(optional)catalog:,(optional)index}"},null);
			return;		
		}
		...
		db.select(params.catalog, params.wid, function(err, doc){
			...
			if(!err && doc){
				...
				if(util.get_field(doc,params.fname)){
					...
					try{										
						util.set_field(doc, params.fname, params.value, params.index );
					}catch(err){
	 ...
	 */
	push : function(params, ret_handler){
	 ...
	 */
	pop : function(params, ret_handler){
	 ...
	 */
	shift : function(params, ret_handler){
