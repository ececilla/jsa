...
exports.remote.create = function( params, ret_handler ){
	...
	if( typeof params.doc !== "object"){
		...
		if(ret_handler)		
			ret_handler({code:-2, message:"Wrong parameter doc: must be an object"},null);
		else
	...
	async.series([
		function(next){
			...
			if(params.doc.rcpts && exports.rcpts){
				...
				exports.rcpts( params.doc, db, function(rcpts){
...
/*
 * Remove a recipient by uid to the document's rcpts list.
 */
	...
	db.select(params.catalog, params.wid, function(err, doc){
		...
		if(!err && doc){
			if(doc[params.fname]){
				...
				if(doc[params.fname].push)				
					doc[params.fname].push(params.value);
				else{
	...
	db.select(params.catalog, params.wid, function(err, doc){
		...
		if(!err && doc){
			if(doc[params.fname]){
				...
				if(doc[params.fname].pop)				
					doc[params.fname].pop();
				else{
					ret_handler({code:-4, message:"Field '" + params.fname+  "' not an array"},null);
					return;
				}	
	...
	db.select(params.catalog, params.wid, function(err, doc){
		...
		if(!err && doc ){
			if(doc[params.fname]){
				...
				if(doc[params.fname].shift)				
					doc[params.fname].shift();
				else{
					ret_handler({code:-4, message:"Field '" + params.fname+  "' not an array"},null);
					return;
				}	
