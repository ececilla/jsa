...
exports.remote = {
	...
	search: function(ctx, ret_handler){
		...
		if(ctx.params.keywords){
			...
			if( util.is_array(ctx.params.keywords) ){
				
				search_array = search_array.concat( _.map(ctx.params.keywords,function(keyword){
					
					return {keywords:keyword};
				}));
				
			}else
				search_array.push({keywords:ctx.params.keywords});	
		}
		
		if(ctx.params.hashtags){
			if(util.is_array(ctx.params.hashtags)){
				
				search_array = search_array.concat( _.map(ctx.params.hashtags,function(hashtag){
					
					return {hashtags:hashtag};
				}));
			}else
				search_array.push({hashtags:ctx.params.hashtags});
		}
		
		db.criteria(ctx.params.catalog, {$and:search_array},{},function(err,docs){
	...
	/*
	 * list documents stored in a catalog matching a specific criteria, ex:
	 * db.waves.find( { loc:{$within:{$box:[[0,0],[3,42]]}}, keywords:"busco"} );
	 * db.waves.find( { loc:{$within:{$box:[[0,0],[3,42]]}}, $and:[{keywords:"busco"},{hashtags:"#mecanica2?"}] } )
	 * jsa.api.list({catalog:"waves",criteria:[{keywords:"profesor"},{hashtags:"#mecanica2"},{loc:{$within:{$box:[[0,0],[1,1]]}}}]});
	 */
