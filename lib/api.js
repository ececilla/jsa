...
exports.remote = {
	...
	join: function( ctx, ret_handler ){
		...
		if( !util.has_joined(ctx.doc.rcpts, ctx.user.push_id)){
			
			
			//change "gcm" by srv.evmngr.events.push_default_type			
			ctx.doc.rcpts.push({push_id:ctx.user.push_id, push_type:ctx.user.push_type || "gcm"});							
			...
			ret_handler(null,CONST.OK());//user is already joined so send true value back.
		}
	...
	unjoin: function( ctx, ret_handler ){
		...
		ctx.user.wids.splice(ctx.user.wids.indexOf("" + ctx.doc._id),1);//remove wid from user wids list.		
		ret_handler(null,CONST.OK());									
	...
	set: function( ctx, ret_handler){
		...
		if(util.is_array(ctx.params.fname)){
			...
			for( i=0; i< ctx.params.fname.length; i++ ){
				
				util.set_field(ctx.doc, ctx.params.fname[i], ctx.params.value );
			}
		...
		if(ctx.params.fname){//get field/s from document
			...
			if(util.is_array(ctx.params.fname)){ //get all fields passed in and build an structure
				...
				for(i=0; i < ctx.params.fname.length; i++){
					...
					var ivalue = util.get_field(ctx.doc, ctx.params.fname[i]);
					if(ivalue != undefined)
						util.set_field(ret_value,ctx.params.fname[i], ivalue);					
				}
	...
	/*
	 * Search for documents containing a keyword in keywords metadata array.
	 */
	search: function(ctx, ret_handler){
				
		var keyword_array = [];	
		if( util.is_array(ctx.params.keyword) ){
			
			for( i = 0; i < ctx.params.keyword.length; i++ )
				keyword_array.push({keywords:ctx.params.keyword[i]});
		}else
			keyword_array.push({keywords:ctx.params.keyword});	
		
		db.criteria(ctx.params.catalog, {$and:keyword_array},{},function(err,docs){
	...
	/*
	 * list documents stored in a catalog matching a specific criteria
	 */
	list: function(ctx,ret_handler){
				
		var criteria_array = [];
		
		if(!util.is_array(ctx.params.criteria))			
			criteria_array.push(ctx.params.criteria);
		else
			criteria_array = ctx.params.criteria;
		
		db.criteria(ctx.params.catalog, {$and:criteria_array}, {},function(err,docs){
