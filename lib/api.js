...
exports.remote = {
	...
	join: function( ctx, ret_handler ){
		...
		if( !util.has_joined(ctx.doc.rcpts, ctx.user.push_id)){
			...
			var rcpt = {uid: "" + ctx.user._id, push_id:ctx.user.push_id, push_type:ctx.user.push_type || gcm};
	...
	search: function(ctx, ret_handler){
		...
		if(ctx.params.keyword){			
			if( util.is_array(ctx.params.keyword) ){
				...
				search_array = search_array.concat( _.map(ctx.params.keyword,function(keyword){
					
					return {keywords:keyword};
				}));
				
			}else
				search_array.push({keywords:ctx.params.keyword});	
		}
		
		if(ctx.params.hashtag){
			if(util.is_array(ctx.params.hashtag)){
				
				search_array = search_array.concat( _.map(ctx.params.hashtag,function(hashtag){
					
					return {hashtags:hashtag};
				}));
			}else
				search_array.push({hashtags:ctx.params.hashtag});
		}
