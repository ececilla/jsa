...
exports.remote = {
	...
	create: function( ctx, ret_handler ){
		...
		async.series([
			...
			function(next){//update user wids list
				...
				db.update("users",ctx.user._id,{$push:{wids:ctx.doc.wid}},function(err){
	 ...
	 * TODO: stop timer
	 * TODO: delete doc reference from all doc.rcpts users wids.
	 */
	...
	join: function( ctx, ret_handler ){
		...
		if( !rcpt ){
			...
			ctx.config.user.mod_fields.push("wids");					
